...
const getRootError = (
  ...
  const errors = errorCodes.map((error) => {
    switch (error) {
      ...
      case "too-many-files":
        return `max ${limits.maxFiles} file`
    }
  })
...
const useDropzone = <TUploadRes, TUploadError = string>(
  ...
  const uploadFileRef = useRef<
    (file: File, id: string, tries?: number) => Promise<void>
  >(async () => {})
    ...
    async (file: File, id: string, tries = 0) => {
      ...
      if (result.status === "error") {
        if (autoRetry === true && tries < (maxRetryCount ?? Infinity)) {
          ...
          return uploadFileRef.current(file, id, tries + 1)
        }
  ...
  ({ className, children, ...props }, forwardedRef) => {
    ...
    return (
      <div
        ...
        aria-label="dropzone"
        className={cn(
          "flex items-center justify-center rounded-md border border-input bg-background px-4 py-2 ring-offset-background focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:outline-none",
...
type DropzoneTriggerProps = Omit<ButtonProps, "asChild" | "type">

const DropzoneTrigger = forwardRef<HTMLLabelElement, DropzoneTriggerProps>(
  (
    { className, variant = "secondary", size = "default", children, ...props },
    ref
  ) => {
    const context = useDropzoneContext()
    if (!context) {
    ...
    return (
      <Button
        ...
        {...props}
      >
        <label ref={ref} htmlFor={context.inputId}>
          {children}
          <input
...
export {
  ...
  DropzoneTrigger,
  InfiniteProgress,
  // eslint-disable-next-line react-refresh/only-export-components -- hook is co-located with dropzone primitives
  useDropzone,
}
